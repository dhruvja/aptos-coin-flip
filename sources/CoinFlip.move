module CoinFlip::flip {
    ...
    public entry fun create_room(creator: &signer, admin: address, game_id: u64, bet_amount: u64, choice: u64) acquires GameStore {
    ...
    public entry fun join_room(joinee: &signer, admin: address, game_id: u64) acquires GameStore {
        assert!(exists<GameStore>(admin), EGAME_STORE_DOESNT_EXIST)
    ...
    public entry fun end_to_end(aptos_framework: signer, module_owner: signer, creator: signer, joinee: signer) acquires GameStore {
        ...
        create_room(&creator, module_owner_addr, game_id, bet_amount, choice);
        assert!(coin::balance<AptosCoin>(creator_addr) == (initial_mint_amount - bet_amount), EINVALID_BALANCE);

        join_room(&joinee, module_owner_addr, game_id);
